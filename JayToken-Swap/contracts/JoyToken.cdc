...
pub contract JoyToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @JoyToken.Vault
        ...
        // Destroy the vault
        destroy() {
            JoyToken.totalSupply = JoyToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            JoyToken.totalSupply = JoyToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&JoyToken.Minter>(/public/Minter, target: /storage/MinterStorage)
